...
function productionFor(scenes) { let cursor=0; return scenes.map(s=>{ const start=cursor; cursor+=s.duration; const a=s.assignment; return { ...s, start, end:cursor, aiBackgroundPrompt:s.backgroundPrompt || backgroundPromptFor(s), videoEffect:a?.primary?'Slow parallax across the selected card art with restrained light movement.':'Slow atmospheric drift with a gentle push-in.', audio:s.event?`Low stone, wind, and distant metal textures beneath the narration of ${s.event.slice(0,90)}.`:'Low atmospheric bed beneath narration.', transition:'Soft dissolve timed to the end of the beat.' }; }); }
